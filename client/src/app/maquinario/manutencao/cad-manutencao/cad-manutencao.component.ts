import { DatePipe } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import { MatDialogRef, MatSnackBar } from '@angular/material';
import { Manutencao } from 'src/app/models/maquinario/manutencao';
import { Maquina } from 'src/app/models/maquinario/Maquina';
import { AbastecimentoService } from 'src/app/service/maquinario/abastecimento.service';
import { ManutencaoService } from 'src/app/service/maquinario/manutencao.service';
import { MaquinaService } from 'src/app/service/maquinario/maquina/maquina.service';


@Component({
  selector: 'app-cad-manutencao',
  templateUrl: './cad-manutencao.component.html',
  styleUrls: ['./cad-manutencao.component.css']
})
export class CadManutencaoComponent implements OnInit {
  isNew = true;
  form: FormGroup;
  datepipe: DatePipe;

  maquinas: Array<Maquina>;
  manutencao: Manutencao;

  constructor(
    private maquinaService: MaquinaService,
    private manutencaoService: ManutencaoService,
    private _snackBar: MatSnackBar,
    public dialogRef: MatDialogRef<CadManutencaoComponent>
  ){
    this.form = new FormGroup({
      maquina: new FormControl('', Validators.required),
      data:  new FormControl('', Validators.required),  
      horimetro:  new FormControl(''),  
      descricao:  new FormControl('', Validators.required),  
      valor:  new FormControl('', Validators.required),  
    });
  }

  ngOnInit() {

    this.datepipe = new DatePipe('pt-BR');


    this.loadMaquinas();

  }

  loadMaquinas(){
    this.maquinaService.getAll().subscribe(result =>{
      this.maquinas = result;
    });
  }

  save(){
    this.manutencao = new Manutencao(
      this.form.value.maquina.id,
      this.form.value.descricao,
      this.datepipe.transform(this.form.value.data, 'yyyy-MM-dd'),
      this.form.value.horimetro,
      this.form.value.valor
    );

    this.manutencaoService.create(this.manutencao).subscribe(result =>{
      console.log(result);
      this._snackBar.open("Salvo Com sucesso", "Fechar", { duration: 3000 });
      this.dialogRef.close();
    })
  }

  update(){
    console.log(this.form.value.nome)
    this.manutencao.descricao = this.form.value.descricao;
    this.manutencao.data = this.datepipe.transform(this.form.value.data, 'yyyy-MM-dd');
    this.manutencao.horimetro = this.form.value.horimetro;
    this.manutencao.valor = this.form.value.valor;
    this.manutencao.maquina_id = this.form.value.maquina.id;

    this.manutencaoService.update(this.manutencao.id, this.manutencao).subscribe(result => {
      console.log(result);
      this._snackBar.open("Editado Com sucesso", "Fechar");
      this.dialogRef.close();
    })
  }

  editData(data: Manutencao){
    this.isNew = false;
    this.manutencao = data;
    console.log(data)
    this.form.patchValue(this.manutencao);
  }


}
